...
interface SellConfirmModalProps {
  ...
  marketSymbol?: string;
  /**
   * When true, the modal always tells the user that any active TP/SL on the
   * symbol will be cancelled — regardless of whether the sell is full or
   * partial. Set by callers (Dashboard, Top-trades) that pair this modal
   * with the backend's `cancelOpenOrders` flag, so the displayed copy
   * matches the actual server-side behavior. Leave false for callers that
   * sell without touching open orders.
   */
  cancelsOpenOrdersNote?: boolean;
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Two-tier intro: a neutral one-liner about what's being sold, then
            (when applicable) an amber warning callout about TP/SL being
            cancelled. Splitting them lets the warning carry color/iconography
            without making the whole intro look alarming. */}
        <p className="mb-3 text-sm text-slate-300">
          {isFullClose
            ? "You're closing this position fully at the current market price."
            : "You're selling part of this position at the current market price."}
        </p>
        {(isFullClose || cancelsOpenOrdersNote) && (
          <div className="mb-5 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-xs text-amber-200">
            <svg
            ...
            <span>
              {isFullClose
                ? "Any active Take-Profit / Stop-Loss on this position will be cancelled first."
                : "Any active Take-Profit / Stop-Loss on this position will be cancelled first — the remaining shares will not be protected."}
            </span>
          </div>
        )}

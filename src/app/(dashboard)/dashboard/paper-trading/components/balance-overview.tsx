...
export function BalanceOverview({ balance, openOrdersCount, loading, isStockMode = false }: BalanceOverviewProps) {
  return (
    <div className={`rounded-lg sm:rounded-xl bg-gradient-to-br from-white/[0.07] to-transparent p-4 sm:p-6 backdrop-blur shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_0_20px_${isStockMode ? 'rgba(59,130,246,0.08)' : 'rgba(252,79,2,0.08)'}]`}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-0">
        <div>
          <p className="text-xs sm:text-sm text-slate-400 mb-1">
            {isStockMode ? 'Paper Trading Balance' : 'Testnet Balance'}
          </p>
          {loading ? (
      ...
      <div className="mt-3 sm:mt-4 flex items-center gap-2 text-xs text-slate-400">
        <div className={`h-2 w-2 rounded-full ${isStockMode ? 'bg-blue-500' : 'bg-green-500'} ${isStockMode ? '' : 'animate-pulse'}`} />
        <span>
          {isStockMode 
            ? 'Stock Paper Trading (Alpaca integration coming soon)' 
